...
export default function Index() {
  ...
  useEffect(() => {
    IDB.get(params.id)
      .then((note) => {
        if ($ref.current) {
          const { root, title } = note
          const data = $ref.current.parseEditorState({ root })

          $ref.current.setEditorState(data)
          $title.current?.setHTMLUnsafe(title)
        }
      })
  ...
  return (
    <div className="absolute top-2 start-2 rounded-lg bg-gold shadow-outer sm:w-1/2 md:w-1/3 p-1 text-1xl">
